...
import { addPageNumbers } from '../../../../lib/pdf-processor.js';
...
export async function POST(request) {
  ...
  try {
    ...
    const result = await addPageNumbers(buf, { position: fields.position, startFrom: parseInt(fields.startFrom??'1',10), fontSize: parseInt(fields.fontSize??'12',10), prefix: fields.prefix??'' });
    return buildFileResponse(result,'application/pdf','numbered.pdf');
  } catch(err) { return buildErrorResponse(err,'add-page-numbers'); }
}
