...
export async function POST(request){
  ...
  try{
    ...
    const result=await convertPdfToOffice(buf,'xlsx',filename);
    const mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
    return buildFileResponse(result,mime,filename.replace(/\.pdf$/i,'.xlsx'));
  }catch(err){return buildErrorResponse(err,'pdf-to-xlsx');}
}
