...
export async function POST(request){
  ...
  try{
    const{buf,filename}=await new Promise(async(resolve,reject)=>{
      ...
      let buf=null,filename='file.xlsx';
      bb.on('file',(_,s,info)=>{filename=info.filename||filename;const c=[];s.on('data',d=>c.push(d));s.on('end',()=>{buf=Buffer.concat(c);});});
      bb.on('finish',()=>{if(!buf)return reject(new ConversionError(400,'No file uploaded.'));resolve({buf,filename});});
    ...
    const result=await convertOfficeToPdf(buf,ext,filename);
    return buildFileResponse(result,'application/pdf',filename.replace(/\.[^.]+$/,'.pdf'));
  }catch(err){return buildErrorResponse(err,'xlsx-to-pdf');}
}
