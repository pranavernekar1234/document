  ...
  return (
    <div style={{ background:'linear-gradient(180deg,#080e1c 0%,#0F172A 100%)', minHeight:'100vh' }}>
      <div className="relative overflow-hidden border-b border-white/5">
        ...
        <div className="absolute inset-0 pointer-events-none" style={{ background:'radial-gradient(ellipse 50% 60% at 50% -10%,rgba(245,158,11,0.18),transparent)' }} />
        <div className="relative max-w-6xl mx-auto px-6 py-10">
          ...
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background:'rgba(245,158,11,0.15)',border:'1px solid rgba(245,158,11,0.3)',color:ACCENT }}>
              <Edit3 size={22}/>
            </div>
            <div>
              <div className="flex items-center gap-2.5 mb-1">
                <h1 className="font-black text-white text-2xl sm:text-3xl tracking-tight">Edit PDF</h1>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full text-white" style={{ background:'rgba(245,158,11,0.25)',border:'1px solid rgba(245,158,11,0.35)' }}>FREE</span>
              </div>
              <p className="text-slate-400 text-base">Annotate, highlight, redact, and add text directly on your PDF pages.</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {state === 'success' && result && (
            <motion.div key="success" initial={{ opacity:0,y:8 }} animate={{ opacity:1,y:0 }} exit={{ opacity:0 }}
              className="rounded-3xl flex flex-col items-center justify-center gap-6 px-8 py-20"
              ...
              <div className="text-center">
                <p className="font-black text-xl text-white">Edits applied!</p>
                <p className="text-slate-400 text-sm mt-1">{result.filename} · {formatFileSize(result.size)} · downloaded automatically</p>
              </div>
          ...
          {state === 'error' && (
            <motion.div key="error" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
              className="rounded-3xl flex flex-col items-center justify-center gap-5 px-8 py-20"
              style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', minHeight:360 }}>
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ background:'rgba(244,63,94,0.1)',border:'1px solid rgba(244,63,94,0.25)' }}>
                <AlertCircle size={24} className="text-rose-400"/>
              </div>
              <div className="text-center max-w-sm"><p className="font-bold text-white">Something went wrong</p><p className="text-slate-400 text-sm mt-1.5">{error}</p></div>
              ...
              <div
                ...
                <div className="flex flex-col items-center gap-4 p-8 select-none">
                  <motion.div animate={{ y:[0,-8,0] }} transition={{ duration:4.5,repeat:Infinity,ease:'easeInOut' }}
                    className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ background:'rgba(245,158,11,0.15)',border:'1px solid rgba(245,158,11,0.3)',color:ACCENT }}>
                    <Upload size={26}/>
                  </motion.div>
                  <div className="text-center">
                    <p className="font-semibold text-white text-[16px]">Drop your PDF to start editing</p>
                    <p className="text-slate-500 text-sm mt-1.5">or <span className="font-medium cursor-pointer" style={{color:ACCENT}}>click to browse</span> · Max 50 MB</p>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          {state === 'loading' && (
            <motion.div key="loading" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
              className="rounded-3xl flex flex-col items-center justify-center gap-5 px-8 py-24"
              style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', minHeight:400 }}>
              <div className="w-16 h-16 rounded-full border-4 border-amber-500/20 border-t-amber-500 animate-spin" />
              <p className="text-white font-semibold">Loading PDF preview…</p>
            </motion.div>
          )}

          {(state === 'uploading' || state === 'processing') && (
            <motion.div key="processing" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
              className="rounded-3xl flex flex-col items-center justify-center gap-7 px-8 py-24"
              style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', minHeight:400 }}>
              <div className="relative" style={{ width:72,height:72 }}>
                <svg className="w-full h-full -rotate-90" viewBox="0 0 80 80" fill="none">
                  ...
                  <circle cx="40" cy="40" r="34" stroke="url(#pg-edit)" strokeWidth="5" strokeLinecap="round"
                  ...
                  <defs><linearGradient id="pg-edit" x1="0" y1="0" x2="80" y2="80" gradientUnits="userSpaceOnUse"><stop stopColor="#F59E0B"/><stop offset="1" stopColor="#EF4444"/></linearGradient></defs>
                </svg>
              ...
              <p className="text-white font-semibold text-lg">{state==='uploading'?'Uploading…':'Applying your edits…'}</p>
            </motion.div>
          )}
            ...
            <motion.div key="editing" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}>
              <div className="grid lg:grid-cols-[1fr_240px] gap-5">
                ...
                <div className="rounded-3xl overflow-hidden" style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)' }}>
                  {/* Toolbar */}
                  <div className="flex items-center gap-2 p-4 border-b border-white/6 flex-wrap">
                    {[['highlight','Highlight', <Highlighter size={14} key="h"/>],['redact','Redact', <Square size={14} key="r"/>],['text','Text', <Type size={14} key="t"/>]].map(([v,l,ic]) => (
                      <button key={v} onClick={()=>setTool(v)}
                        ...
                        style={{ background:tool===v?'rgba(245,158,11,0.2)':'rgba(255,255,255,0.04)', border:`1px solid ${tool===v?'rgba(245,158,11,0.4)':'rgba(255,255,255,0.08)'}`, color:tool===v?'#FBB740':'#94A3B8' }}>
                        {ic} {l}
                      </button>
                    ))}
                    {tool === 'highlight' && (
                      <div className="flex items-center gap-1.5 ml-1">
                        {['yellow','green','blue','pink'].map(c => (
                          <button key={c} onClick={()=>setHighlightColor(c)}
                            className={cn('w-6 h-6 rounded-full transition-transform', highlightColor===c && 'scale-110 ring-2 ring-white/40')}
                            style={{ background: { yellow:'#FDE047', green:'#86EFAC', blue:'#93C5FD', pink:'#F9A8D4' }[c] }} />
                        ))}
                      </div>
                    )}
                    <div className="flex-1" />
                    <button onClick={undoLast} disabled={!annotations.length} className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-white/5 disabled:opacity-30 transition-colors" aria-label="Undo last">
                      <RotateCcw size={15}/>
                    </button>
                    <button onClick={clearPage} disabled={!annotations.filter(a=>a.page===pageNum-1).length} className="p-2 rounded-lg text-slate-400 hover:text-rose-400 hover:bg-white/5 disabled:opacity-30 transition-colors" aria-label="Clear page">
                      <Trash2 size={15}/>
                    </button>
                  </div>

                  {/* Canvas area */}
                  <div className="p-5 flex flex-col items-center gap-4 overflow-auto" style={{ maxHeight: 640 }} ref={containerRef}>
                      ...
                      {textInput && (
                        ...
                        >
                          <input
                            ...
                            style={{ minWidth: 140, border: '2px solid #F59E0B' }}
                          />
                          <button onClick={confirmText} className="p-1 rounded bg-emerald-500 text-white"><Check size={12}/></button>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between px-5 py-3.5 border-t border-white/6">
                    <button onClick={()=>setPageNum(p=>Math.max(1,p-1))} disabled={pageNum<=1}
                      className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-white/5 disabled:opacity-30 transition-colors"><ChevronLeft size={16}/></button>
                    <span className="text-slate-400 text-sm">Page {pageNum} of {numPages}</span>
                    <button onClick={()=>setPageNum(p=>Math.min(numPages,p+1))} disabled={pageNum>=numPages}
                      className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-white/5 disabled:opacity-30 transition-colors"><ChevronRight size={16}/></button>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="rounded-2xl p-5" style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)' }}>
                    <p className="text-white font-semibold text-sm mb-3">{file?.name}</p>
                    <p className="text-slate-500 text-xs mb-4">{numPages} page{numPages!==1?'s':''} · {annotations.length} annotation{annotations.length!==1?'s':''}</p>
                    <motion.button whileTap={{ scale: annotations.length ? 0.97 : 1 }} onClick={applyEdits} disabled={!annotations.length}
                      className={cn('w-full flex items-center justify-center gap-2 rounded-xl px-4 py-3 font-bold text-sm text-white', !annotations.length && 'opacity-40 cursor-not-allowed')}
                      style={{ background: annotations.length ? `linear-gradient(135deg,${ACCENT},#EF4444)` : 'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.12)' }}>
                  ...
                  <div className="rounded-2xl p-4" style={{ background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)' }}>
                    <p className="text-slate-500 text-xs leading-relaxed">
                      <strong className="text-slate-300">Highlight:</strong> drag to mark text.<br/>
                      <strong className="text-slate-300">Redact:</strong> drag to permanently black-out content.<br/>
                      <strong className="text-slate-300">Text:</strong> click and type to add a label.
                    </p>
                  </div>
                  <button onClick={reset} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold transition-all" style={{ background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',color:'#94A3B8' }}>
