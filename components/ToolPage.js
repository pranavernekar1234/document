      ...
      <div className="relative overflow-hidden border-b border-black/5">
        ...
        <div className="relative max-w-5xl mx-auto px-6 py-10">
          ...
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 bg-black/5 text-black">
              {tool.icon}
            </div>
            <div>
              <div className="flex items-center gap-2.5 mb-1">
                <h1 className="font-semibold text-black text-2xl sm:text-3xl tracking-tight">{tool.name}</h1>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full text-white bg-black">FREE</span>
              </div>
              <p className="text-black/55 text-base">{tool.description}</p>
            </div>
          </div>
        </div>
      </div>
              ...
              {!isProcessing && state !== 'success' && state !== 'error' && (
                <motion.div key="idle" variants={fadeScale} initial="hidden" animate="show" exit="exit">
                  <IdlePanel
                    ...
                    options={options ? <div className="mt-6 border-t border-black/5 pt-6">{options({ values: optValues, setOpt })}</div> : null}
                  />
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <div className="space-y-4">
            {featureBullets.length > 0 && (
              <div className="rounded-2xl p-5 bg-white border border-black/5 shadow-soft">
                <p className="text-black font-semibold text-sm mb-3 flex items-center gap-2">
                  <Settings size={14} className="text-black/50" /> Features
                </p>
                <ul className="space-y-2.5">
                  {featureBullets.map((b, i) => (
                    <li key={i} className="flex items-start gap-2">
                      <div className="flex-shrink-0 mt-0.5 w-4 h-4 rounded-full flex items-center justify-center bg-emerald-50">
            ...
            <div className="rounded-2xl p-4 flex items-start gap-3 bg-emerald-50 border border-emerald-600/15">
              ...
              <div>
                <p className="font-semibold text-emerald-700 text-xs">Privacy guaranteed</p>
                <p className="text-emerald-700/70 text-[11px] mt-0.5 leading-relaxed">Files processed in-memory and permanently discarded. Zero storage.</p>
              </div>
            </div>
            <div className="rounded-2xl p-4 bg-[#FAFAFA] border border-black/5">
              <p className="text-black/35 text-[10px] font-semibold uppercase tracking-wider mb-2.5">Limits</p>
              <div className="space-y-2">
                {[['Max size','50 MB'],['Cost','Free'],['Timeout','5 min']].map(([l,v]) => (
                  <div key={l} className="flex items-center justify-between">
                    <span className="text-black/45 text-xs">{l}</span>
                    <span className="text-black text-xs font-semibold">{v}</span>
                  </div>
                ))}
              </div>
            </div>
  ...
  return (
    <div className="p-7 sm:p-9">
      ...
      <div
        ...
        style={{ minHeight: hasFiles ? 140 : 220, background: isDragover ? 'rgba(0,0,0,0.035)' : '#FAFAFA' }}
      ...
      >
        ...
        {hasFiles ? (
          <div className="flex items-center gap-3 px-4 py-3 w-full justify-center">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center bg-black/5 text-black">
              <FileText size={15} />
            </div>
            <span className="text-black/70 text-sm font-medium">{files.length} file{files.length > 1 ? 's' : ''} selected</span>
            <button className="ml-2 text-xs text-black/35 hover:text-black/60 transition-colors" onClick={e => { e.stopPropagation(); fileInputRef.current?.click(); }}>
              Add more
            </button>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-4 p-8 select-none">
            <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
              className="w-16 h-16 rounded-2xl flex items-center justify-center bg-black/5 text-black">
              <Upload size={26} />
            </motion.div>
            <div className="text-center">
              ...
              <p className="text-black/45 text-sm mt-1">
                or <span className="font-medium cursor-pointer text-black">click to browse</span> · Max 50 MB
              </p>
            </div>
          </div>
        )}
        ...
        <div className="space-y-2 mb-5">
          {files.map((f, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
              className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-[#FAFAFA] border border-black/5">
              <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 bg-black/5 text-black">
                <FileText size={13} />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-black text-sm font-medium truncate">{f.name}</p>
                <p className="text-black/40 text-xs">{formatFileSize(f.size)}</p>
              </div>
              <button onClick={() => onRemoveFile(i)} className="text-black/30 hover:text-black/60 transition-colors p-1" aria-label="Remove file">
                <X size={14} />
              </button>
            </motion.div>
          ))}
          {tool.multiple && files.length < (tool.maxFiles ?? 20) && (
            <button onClick={() => fileInputRef.current?.click()}
              className="w-full flex items-center justify-center gap-2 py-2 rounded-xl text-sm text-black/60 hover:text-black transition-colors border border-dashed border-black/20 hover:border-black/35">
      ...
      {/* Action button */}
      <motion.button whileTap={{ scale: files.length ? 0.97 : 1 }} whileHover={{ scale: files.length ? 1.01 : 1 }} onClick={onProcess} disabled={!files.length} aria-disabled={!files.length}
        className={cn('w-full flex items-center justify-center gap-2.5 rounded-full px-6 py-4 font-medium text-[15px] tracking-tight transition-all duration-200 mt-5',
          files.length ? 'bg-black text-white hover:bg-[#222]' : 'bg-black/5 text-black/30 cursor-not-allowed')}>
    ...
    <div className="flex flex-col items-center justify-center gap-8 px-8 py-24" style={{ minHeight: 360 }}>
      <div className="relative w-20 h-20">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="font-semibold text-sm text-black">{isUploading ? `${progress}%` : '…'}</span>
        </div>
      </div>
      <div className="text-center">
        <p className="font-semibold text-black text-lg">{isUploading ? 'Uploading…' : `Processing with ${toolName}…`}</p>
        <p className="text-black/45 text-sm mt-1.5">{isUploading ? `${progress}% transferred` : 'This may take a few seconds'}</p>
      </div>
      ...
      {isUploading && <button className="text-xs text-black/35 hover:text-rose-600 transition-colors" onClick={onCancel}>Cancel</button>}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-6 px-8 py-20" style={{ minHeight: 360 }}>
      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 260, damping: 20, delay: 0.1 }}
        className="rounded-full flex items-center justify-center bg-emerald-50"
      ...
      <div className="text-center">
        ...
        {result?.parts ? (
          <p className="text-black/50 text-sm mt-1">{result.parts.length} files ready for download</p>
        ) : (
          <p className="text-black/50 text-sm mt-1">Your file is ready</p>
        )}
      </div>
        ...
        <div className="w-full max-w-sm space-y-2">
          {result.parts.map((p, i) => (
            <button key={i} onClick={() => downloadPart(p)}
              className="w-full flex items-center justify-between px-4 py-2.5 rounded-xl text-sm transition-all hover:bg-black/[0.06] bg-black/[0.03] border border-black/5 text-black">
      ...
      <button onClick={onReset}
        className="flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium transition-all border border-black/10 text-black hover:bg-black/[0.03]">
...
function ErrorPanel({ message, onReset }) {
  return (
    <div className="flex flex-col items-center justify-center gap-5 px-8 py-20" style={{ minHeight: 360 }}>
      ...
      <button onClick={onReset}
        className="flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium transition-all bg-rose-50 text-rose-700 border border-rose-100 hover:bg-rose-100">
        Try again
      </button>
    </div>
  );
